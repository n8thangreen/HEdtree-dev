...
##' 
##' Generate a string corresponding to calculating the mean tree output for a given quantity over the tree
##' The tree is specified as the input \code{node}, which must be a \code{data.tree}
...
getAQ <- function(node, qnt='cost') {
    ...
    if(!node$isRoot & is.null(node$p))
      stop(paste0('Probability not found for node:', node$name,'!'), call. = FALSE)
